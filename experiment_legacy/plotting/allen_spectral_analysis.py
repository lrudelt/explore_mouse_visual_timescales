...
def load_spike_data_hdf5_per_area(filepath, session_id, stimulus, stimulus_block, area):
    filename = f"{filepath}/session_{session_id}/session_{session_id}_spike_data.h5"
    metadata = pd.read_hdf(filename, f"session_{session_id}_stimulus_{stimulus}_stimulus_block_{stimulus_blocks[0]}_metadata")
    f = h5py.File(os.path.expanduser(filename), "r", libver="latest")
    spike_times = f[f"session_{session_id}_stimulus_{stimulus}_stimulus_block_{stimulus_block}_spiketimes"]
    n_units = len(metadata.unit_id.values)
    spike_times_area = [] 
    for i in range(n_units):
        if metadata["ecephys_structure_acronym"][i] == area:
           spike_times_area += [spike_times[i]]
    spike_times_area = np.concatenate(spike_times_area)
    spike_times_area = np.sort(spike_times_area)
    return spike_times_area[np.isfinite(spike_times_area)]
...
def get_power_spectrum_area(spike_times,
        min_omega = 0.003,
        max_omega = 10,
        n_bins = 50,
        n_snippets = 4,
    ...
    power_averaged = np.zeros(n_bins)
    T_recording = spike_times[-1] - spike_times[0]
    T_snippet = T_recording/n_snippets
    T_snippet_list = np.random.uniform(0.7, 1.3, size = n_snippets) * T_snippet
    T_snippet_list = T_snippet_list * T_recording / np.sum(T_snippet_list)
    for i in range(n_snippets):
        if i > 0:
            ...
            T_low = 0.
        T_up = np.sum([T_snippet_list[j] for j in range(i+1)])
        selection = spike_times >= spike_times[0] + T_low
        selection &= spike_times < spike_times[0] + T_up
        # print(spike_times[(spike_times >= i*T_snippet)])
        spikes = spike_times[selection]
        n_snippets_eff = 0
        if len(spikes) > 0:
            power, omegas = power_spectrum(spikes, min_omega, max_omega, n_bins)
            power_averaged += power
    power_averaged = power_averaged / n_snippets_eff
    # plot_spectrum(power, omegas)
    return power_averaged, omegas
...
if __name__ == "__main__":
    ...
    # unit_index = 50
    fig, ax = plt.subplots(figsize = [3.1,2.3])#figsize=plot_settings["panel_size"]) # FIG SIZE: 3.1, 2.1
    ...
    for unit_index in range(n_units):
        ...
        norm = np.sum(power)
        ax.plot(omegas, power/norm, color = 'cornflowerblue', alpha = 0.07, lw = 0.4)
    norm = np.sum(1/omegas)
    ax.plot(omegas, 1/omegas/norm, color = "0.4", zorder = 300, lw = 1.5, label = "1/f")
    norm = np.sum(np.exp(log_mean_power))
    ax.plot(omegas, np.exp(log_mean_power)/ norm, color = 'cornflowerblue', lw = 2)
    ax.legend()
    ax.set_title(f"mouse {int(argv[2])+1}, visual cortex")
            # print(metadata)
    # load settings
    ...
    # csv_output_dir = analysis_settings["csv_output_dir"]bbox_inches=None,
    plt.savefig(f"{img_dir}/{__file__[:-3]}_session_{session_id}_{stimulus}.png", format = "png", dpi = 500, bbox_inches='tight')
    exit()
